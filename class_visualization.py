...
class ClassVisualization:
    ...
    def create_class_visualization(self, target_y, class_names, model, **kwargs):
        ...
        def compute_gradient(model, img_var):
            score = model(img_var)[0, target_y]
            reg = l2_reg * torch.linalg.norm(img_var)**2
            update = score - reg

            with torch.no_grad():
                update.backward()
                img_var += learning_rate * img_var.grad / torch.linalg.norm(img_var.grad)
                img_var.grad.zero_()
                # torch.add(img_var.data, (learning_rate * update))
                # print("update", learning_rate * update)
