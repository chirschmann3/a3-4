...
def style_transfer(name, content_image, style_image, image_size, style_size, content_layer, content_weight,
    ...
    for t in range(200):
        ...
        ##############################################################################
        # get losses
        cont_loss = content_loss(content_weight, feats[content_layer], content_target)
        sty_loss = style_loss(feats, style_layers, style_targets, style_weights)
        t_loss = tv_loss(img_var, tv_weight)
        total_loss = cont_loss + sty_loss + t_loss
        optimizer.zero_grad()
        total_loss.backward()
